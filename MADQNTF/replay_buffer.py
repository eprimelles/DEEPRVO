...
class Replay_Buffer:
    ...
    def __init__(self, state_space, n_agents, max_length, batch_size) -> None:
        self.state_buffer = np.zeros((max_length, n_agents, state_space), dtype=np.float32)
        self.action_buffer = np.zeros((max_length, n_agents), dtype=np.int32)
        self.state_1_buffer = np.zeros((max_length, n_agents, state_space), dtype=np.float32)
